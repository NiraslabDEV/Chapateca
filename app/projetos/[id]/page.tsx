...
import { MapPin, Calendar, Users, Download, ArrowLeft } from 'lucide-react'
import DownloadAllProjeto from '@/components/galeria/download-all-projeto'

export default async function PublicProjetoPage({
  params,
}: {
    ...
    return (
      <div className="min-h-screen flex items-center justify-center bg-[#FAF8F4]">
        <div className="text-center px-6">
          ...
          <h1 className="text-xl font-semibold text-gray-800 mb-2">Projecto não encontrado</h1>
          <p className="text-sm text-gray-500 mb-6">Este projecto pode ter sido removido ou está privado.</p>
          <Link href="/projetos" className="inline-flex items-center gap-2 px-5 py-2.5 bg-purple-700 text-white text-sm font-semibold rounded-full hover:bg-purple-800 transition-colors">
            <ArrowLeft size={14} /> Ver todos os projectos
          </Link>
        </div>
      </div>
    )
  ...
  const title = album.activityName || `Actividade · ${album.location}`
  const formattedDate = album.activityDate.toLocaleDateString('pt-MZ', {
    day: 'numeric', month: 'long', year: 'numeric',
  })
  const realPhotos = album.photos.filter(p => !p.googleDriveId.startsWith('mock-'))
    ...
    <div className="min-h-screen bg-[#FAF8F4]">
      ...
      <div className="bg-white border-b border-gray-100">
        <div className="max-w-5xl mx-auto px-4 sm:px-6 py-3">
          <Link href="/projetos" className="inline-flex items-center gap-1.5 text-[13px] text-gray-500 hover:text-purple-700 transition-colors">
            <ArrowLeft size={14} /> Todos os projectos
          </Link>
        </div>
      </div>

      {/* Header */}
      <div className="px-6 py-12 text-white text-center" style={{ background: 'linear-gradient(135deg, #461882 0%, #6B2D10 100%)' }}>
        <div className="max-w-2xl mx-auto">
          ...
          <div className="flex flex-wrap items-center justify-center gap-4 text-sm opacity-85">
            <span className="flex items-center gap-1.5"><MapPin size={14} />{album.location}</span>
            <span className="w-1 h-1 rounded-full bg-white/50" />
            <span className="flex items-center gap-1.5"><Calendar size={14} />{formattedDate}</span>
            {album.participants && (
              <>
                <span className="w-1 h-1 rounded-full bg-white/50" />
                <span className="flex items-center gap-1.5"><Users size={14} />{album.participants} participantes</span>
              </>
            )}
          </div>
          {album.activityType && (
          ...
          {realPhotos.length > 0 && (
            <div className="mt-6">
              <DownloadAllProjeto
                albumId={album.id}
                urls={realPhotos.map(p => p.googleDriveId)}
              />
            </div>
          )}
        </div>
      ...
      <div className="max-w-5xl mx-auto px-4 py-10">
        {album.photos.length > 0 ? (
            ...
            {album.photos.map((photo, idx) => {
              ...
              return (
                <div key={photo.id} className="group relative aspect-square rounded-xl overflow-hidden bg-gray-100 shadow-sm">
                  {isMock ? (
                    <div className="w-full h-full flex items-center justify-center" style={{ background: MOCK_COLORS[idx % MOCK_COLORS.length] }}>
                      <span className="text-[10px] text-white/60 font-mono uppercase tracking-wider px-2 py-1 bg-black/20 rounded">foto</span>
                    </div>
                  ) : (
                    <>
                      {/* eslint-disable-next-line @next/next/no-img-element */}
                      <img
                        src={`/api/projeto/${album!.id}/${photo.googleDriveId}?w=600`}
                        alt={photo.fileName}
                      ...
                      <div className="absolute inset-0 bg-black/40 opacity-0 group-hover:opacity-100 transition-opacity flex items-end justify-end p-2">
                        <a
                          href={`/api/projeto/${album!.id}/${photo.googleDriveId}?download=1`}
          ...
          <p className="text-center text-sm text-gray-400 py-12">Nenhuma foto neste projecto.</p>
        )}
      ...
      <footer className="text-center py-8 text-xs text-gray-400 border-t border-gray-100 mt-4">
        ...
        <p className="mt-1">
          <a href="https://www.chapateca.org" target="_blank" rel="noopener noreferrer" className="hover:text-gray-600 transition-colors">
            www.chapateca.org
          </a>
        </p>
      </footer>
