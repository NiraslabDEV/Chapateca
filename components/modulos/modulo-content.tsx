...
import { Upload, Eye, Download, FolderOpen, Clock, X, ExternalLink, FileText, FileSpreadsheet, AlertCircle } from 'lucide-react'
              ...
              {folder.files.length > 0 ? (
                <div className="divide-y divide-sand-light">
                  {folder.files.map(file => (
                    <div key={file.id} className="flex items-center gap-3 px-6 py-3 group hover:bg-parchment-2 transition-colors">
        ...
        <>
          ...
          <div className="fixed right-0 z-50 bg-white border-l border-sand-light shadow-2xl flex flex-col w-full md:w-[500px]"
            ...
            <div className="flex items-center gap-3 px-5 py-4 border-b border-sand-light flex-shrink-0">
              ...
              <button
                onClick={() => setSelectedFile(null)}
                className="w-8 h-8 rounded-xl flex items-center justify-center hover:bg-parchment-2 text-ink-soft transition-colors flex-shrink-0"
              >
                <X size={16} />
              </button>
            </div>
            ...
            <div className="flex-1 min-h-0 bg-parchment">
              {selectedFile.fileType === 'PDF' && !selectedFile.driveId.startsWith('mock-') ? (
                <iframe
                  src={`/api/drive/doc/${selectedFile.driveId}`}
                  ...
                  {selectedFile.driveId.startsWith('mock-') ? (
                    <>
                      ...
                      <div>
                        ...
                        <p className="text-xs text-ink-soft">Este documento não tem ficheiro real associado. Carrega um documento real para visualizar.</p>
                      </div>
                    </>
                  ) : (
                    <>
                      ...
                      <div>
                        ...
                        <p className="text-xs text-ink-soft">
                          Ficheiros {selectedFile.fileType} não podem ser visualizados inline.<br />
                          Usa "Ver documento completo" para abrir no browser.
                        </p>
                      </div>
                    </>
                  )}
            ...
            <div className="flex gap-2 p-4 border-t border-sand-light flex-shrink-0">
              <a
                href={`/api/drive/doc/${selectedFile.driveId}`}
                target="_blank"
              ...
              <a
                href={`/api/drive/doc/${selectedFile.driveId}?download=1`}
